...
st.markdown("<p style='text-align: center; color: gray;'>กรอกข้อมูลสุขภาพของคุณด้านล่าง เพื่อทำนายความเสี่ยงด้วย AI (Decision Tree + StandardScaler)</p>", unsafe_allow_html=True)
st.markdown("---")

# --- ส่วนรับค่า Input (แบ่งเป็น 2 คอลัมน์) ---
...
with col1:
    ...
    cp = st.selectbox("ประเภทอาการเจ็บหน้าอก (ChestPainType)", 
                      ["1: Typical Angina", "2: Atypical Angina", "3: Non-anginal", "4: Asymptomatic"])
    cp_val = int(cp.split(':')[0])
    
    trestbps = st.slider("ความดันโลหิตขณะพัก RestingBP (mm Hg)", 80, 200, 120)
    chol = st.slider("ระดับคอเลสเตอรอล Cholesterol (mg/dl)", 0, 600, 200)
    
    fbs = st.selectbox("น้ำตาลในเลือดขณะอดอาหาร FastingBS > 120 mg/dl?", ["ไม่ (0)", "ใช่ (1)"])
    fbs_val = 1 if "ใช่" in fbs else 0

with col2:
    st.markdown("##### 📋 ผลการตรวจคลื่นหัวใจและการออกกำลังกาย")
    restecg = st.selectbox("ผลคลื่นไฟฟ้าหัวใจขณะพัก RestingECG", 
                           ["0: Normal", "1: ST-T abnormality", "2: LVH"])
    restecg_val = int(restecg.split(':')[0])
    
    thalach = st.slider("อัตราการเต้นของหัวใจสูงสุด MaxHR (bpm)", 60, 220, 150)
    
    exang = st.selectbox("มีอาการเจ็บหน้าอกขณะออกกำลังกาย ExerciseAngina?", ["ไม่ (0)", "ใช่ (1)"])
    exang_val = 1 if "ใช่" in exang else 0
    
    oldpeak = st.slider("ค่า ST depression Oldpeak", 0.0, 6.0, 1.0, 0.1)
...
if st.button("🔍 ประเมินความเสี่ยงทันที"):
    # จัดรูปแบบข้อมูลให้ตรงกับคอลัมน์ใน Heart4.csv และตอนเทรนโมเดล
    ...
    # 🔑 ขั้นตอนสำคัญ: ทำการ Scale ข้อมูลด้วย scaler ที่โหลดมา
...
# --- Footer ---
st.markdown("---")
st.markdown("<p style='text-align: center; color: gray; font-size: 12px;'>© 2026 Heart Disease Predictor | Powered by Decision Tree + StandardScaler + Streamlit</p>", unsafe_allow_html=True)